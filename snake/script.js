...
const game = {
     ...
     */
    start(event) {
        if (this.getElement().classList.length !== 2) {
            ...
            if (this.getElement().classList[1] === GAME_STATUS_PAUSED) {
                this.setGameStatus(GAME_STATUS_STARTED)
...
const snake = {
    ...
    foundSnakePos(nextPosition) {
        const head_snake = this.parts.length - 1
        for (let i of this.parts) {
            if (i !== this.parts[head_snake]) {
                if (i.top === nextPosition.top && i.left === nextPosition.left) {
                    return true
                } else {
